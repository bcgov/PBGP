...
const Step: React.FC<StepProps> = ({ index, step, label, isLast, currentStep }) => {
  return (
    <div key={index} className={!isLast ? 'w-full flex items-center' : 'flex items-center'}>
        ...
        <div className={`hidden md:block absolute top-0 text-center mt-10 w-32 text-xs font-medium uppercase ${currentStep == index + 1?'font-bold text-bcBluePrimary':'text-gray-400'}`}>
            {label}
        </div>
        </div>
        {!isLast ? (
        <div className={`flex-grow  ${currentStep !== index + 1 && 'border-dotted'}  mx-2 border-t-2 border-bcBlueNav`}></div>
        ) : null}
    </div>
  );
};
